...
function cleanUrl(sanitize, base, href) {
	if (sanitize) {
		let prot;
		try {
			prot = decodeURIComponent(unescapeHtml(href)).replace(nonWordAndColonTest, "").toLowerCase();
		} catch (_e) {
			return null;
		}
		if (
...
function outputLink(cap, link, raw, lexer) {
	...
	const title = link.title ? escapeHtml(link.title) : null;
...
class _Tokenizer {
	...
	html(src) {
		...
		if (cap) {
			...
			if (this.options.sanitize) {
				const text = this.options.sanitizer ? this.options.sanitizer(cap[0]) : escapeHtml(cap[0]);
	...
	autolink(src, mangle) {
		...
		if (cap) {
			let text, href;
			if (cap[2] === "@") {
				text = escapeHtml(this.options.mangle ? mangle(cap[1]) : cap[1]);
				href = `mailto:${text}`;
			} else {
				text = escapeHtml(cap[1]);
				href = text;
	...
	url(src, mangle) {
		let cap;
		cap = this.rules.inline.url.exec(src);
		if (cap) {
			let text, href;
			if (cap[2] === "@") {
				text = escapeHtml(this.options.mangle ? mangle(cap[0]) : cap[0]);
				href = `mailto:${text}`;
			} else {
	...
	inlineText(src, smartypants) {
		...
		if (cap) {
			...
			if (this.lexer.state.inRawBlock) {
				...
				text = escapeHtml(this.options.smartypants ? smartypants(cap[0]) : cap[0]);
			}
			return {
				type: "text",
				raw: cap[0],
...
const inline = {
	escape: /^\\([!"#$%&'()*+,\-./:;<=>?@[\]\\^_`{|}~])/,
	autolink: /^<(scheme:[^\s\p{Cc}<>]*|email)>/u,
	url: noopTest,
	tag:
		"^comment" +
...
function mangle(text) {
	...
	for (let i = 0; i < text.length; i++) {
		const ch =
			Math.random() > 0.5 ? `x${text.charCodeAt(i).toString(16)}` : text.charCodeAt(i).toString();
		out += `&#${ch};`;
	}
	return out;
}
...
class _Lexer {
	...
	lex(src) {
		...
		let next = this.inlineQueue.shift();
		while (next) {
	...
	blockTokens(src, tokens = []) {
		...
		while (src) {
			...
			// fences
			token = this.tokenizer.fences(src);
			if (token) {
				src = src.substring(token.raw.length);
				tokens.push(token);
				continue;
			...
			// heading
			token = this.tokenizer.heading(src);
			if (token) {
				src = src.substring(token.raw.length);
				tokens.push(token);
				continue;
			...
			// blockquote
			token = this.tokenizer.blockquote(src);
			if (token) {
				src = src.substring(token.raw.length);
				tokens.push(token);
				continue;
			...
			// table (gfm)
			token = this.tokenizer.table(src);
			if (token) {
				src = src.substring(token.raw.length);
				tokens.push(token);
				continue;
			...
			// lheading
			token = this.tokenizer.lheading(src);
			if (token) {
				src = src.substring(token.raw.length);
				tokens.push(token);
				continue;
	...
	inlineTokens(src, tokens = []) {
		...
		while (src) {
			...
			// code
			token = this.tokenizer.codespan(src);
			if (token) {
				src = src.substring(token.raw.length);
				tokens.push(token);
				continue;
			...
			switch (token.type) {
				...
				case "heading": {
					...
					out += this.renderer.heading(
						...
						headingToken.depth,
						unescapeHtml(this.parseInline(headingToken.tokens, this.textRenderer)),
						this.slugger,
					);
					continue;
				}
...
class Marked {
	...
	#onError(silent, async, callback) {
		return (e) => {
			...
			if (silent) {
				const msg = `<p>An error occurred:</p><pre>${escapeHtml(`${e.message}`, true)}</pre>`;
				if (async) {
