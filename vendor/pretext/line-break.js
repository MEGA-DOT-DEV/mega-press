...
export function walkPreparedLinesRaw(prepared, maxWidth, onLine) {
	...
	for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
		...
		while (i < chunk.endSegmentIndex) {
			...
			const w =
				kind === "tab" ? getTabAdvance(lineW + leadingSpacing, prepared.tabStopAdvance) : widths[i];
		...
		if (hasContent) {
			const finalPaintWidth =
				pendingBreakSegmentIndex === chunk.consumedEndSegmentIndex ? pendingBreakPaintWidth : lineW;
